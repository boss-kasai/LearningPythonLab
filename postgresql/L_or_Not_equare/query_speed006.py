...
from postgresql.L_or_Not_equare.make_table006 import AllInTag, Company, LCompanyTag, Tag
...
def search_companies_by_tag(session: Session, tag_keyword: str):
    ...
    query = (
        ...
        .filter(AllInTag.tag_name == tag_keyword)  # 完全一致
        .distinct()  # 重複を防ぐ
    )
...
def search_companies_by_ltabl(session: Session, tag_keyword: str):
    ...
    query = (
        ...
        .filter(Tag.tag == tag_keyword)  # 完全一致検索
        .distinct()  # 重複を防ぐ
    )
    return [company[0] for company in query.all()]
...
# タグ の完全一致の企業名の検索と処理時間の計測
key_words = ["lcvhnf", "bbvwwa", "zwooxh"]
